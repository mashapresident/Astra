...
numChar = -1  # з першого символа
char = ''  # символи(ще порожній рядок)
lexeme = ''  # лексеми(ще порожній рядок)
...
def indexIdConst(state, lexeme):
    index = 0
    if state == 2:
        index = tableOfId.get(lexeme)
        if index is None:
            index = len(tableOfId) + 1
            tableOfId[lexeme] = index
    if state in (5, 6, 9):
        index = tableOfConst.get(lexeme)
        if index is None:
            index = len(tableOfConst) + 1
            tableOfConst[lexeme] = (tokStateTable[state], index)
    return index
